...
cypherTextDelete = ''
cypherTextDelete += "MATCH (n:PatentBarTestQuestion)-[r:PBTQ_CaseAction]->(m:CaseAction) "
cypherTextDelete += "DELETE r"
graph.run(cypherTextDelete)

cypherTextFind = ''
cypherTextFind += "MATCH (n:PatentBarTestQuestion)-[r:PBTQ_CaseAction]->(m:CaseAction) "
cypherTextFind += "RETURN n.name, m.name"
df = DataFrame(graph.run(cypherTextFind))

df.to_csv (r'data/processed/PBTQ/createRels_PBTQ_CaseAction.csv', index = False, header=True)
